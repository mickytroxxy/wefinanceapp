...
function Dashboard(props) {
  ...
  const { loggedUser,accountBalance,formatToCurrency} = React.useContext(AppContext);
    ...
    <div>
      ...
      <List>
        {['SIGN OUT'].map((text, index) => (
          <ListItem button key={text} style={{margin:5,paddingTop:15,paddingBottom:15,paddingLeft:10,borderRadius:10,width:'90%',border: '2px solid tomato',marginTop:100}}>
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      >
        <Toolbar style={{backgroundColor:'#fff'}}>
          ...
          <Typography noWrap>
            <h4 className="fontBold">Hi, {loggedUser.fname.toUpperCase()}</h4>
          </Typography>
          <Box sx={{ flexGrow: 1 }} />
